...
<br /><br />
	...
	<table class="sale_personal_profile_detail data-table">
		<tr>
			<th colspan="2">
                <?if($arResult["ID"]):?>
				<b><?= str_replace("#ID#", $arResult["ID"], GetMessage("WS_SPPD_PROFILE_NO")) ?></b>
                <?else:?>
                <b><?= str_replace("#ID#", $arResult["ID"], GetMessage("WS_SPPD_PROFILE_NEW")) ?></b>
                <?endif?>
			</th>
		</tr>
		<tr>
...
<!--                --><?//=$arResult["PERSON_TYPE"]["NAME"]?>
		...
		<tr>
			<td width="40%" align="right"><?echo GetMessage("WS_SALE_PNAME")?>:<span class="req">*</span></td>
					...
					<tr>
						...
						<td width="50%">
							...
							<?elseif ($vval["TYPE"]=="LOCATION"):?>
								...
								<select name="<?=$name?>" size="<?echo (IntVal($vval["SIZE1"])>0)?$vval["SIZE1"]:1; ?>">
									<?foreach($vval["VALUES"] as $vvval):?>
										<option value="<?echo $vvval["ID"]?>"<?if (IntVal($vvval["ID"])==IntVal($currentValue) || !isset($currentValue) && IntVal($vvval["ID"])==IntVal($vval["DEFAULT_VALUE"])) echo " selected"?>><?echo $vvval["COUNTRY_NAME"].(!empty($vvval["REGION_NAME"])?" - " . $vvval["REGION_NAME"]:"").(($vvval["CITY_NAME"])?" - ".$vvval["CITY_NAME"]:$vvval["CITY_NAME"])?></option>
									<?endforeach;?>
								</select>
								<?
								endif;
	...
	<br />
	<div>
		<input type="submit" name="save" value="<?echo GetMessage("WS_SALE_SAVE") ?>">
		&nbsp;
		<input type="submit" name="apply" value="<?=GetMessage("WS_SALE_APPLY")?>">
		&nbsp;
		<input type="submit" name="reset" value="<?echo GetMessage("WS_SALE_RESET")?>">
	</div>
	</form>