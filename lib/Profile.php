...
class Profile extends Object{
    ...
    static function Update($profileID, $arFields) {
        ...
        if (empty($profileID)) {
            return $result->addErrorString(Module::get()->getMessage("save_error_required_id"));
        }
        $DB->StartTransaction();
        ...
        if (!empty($arFields)) {
            if(!$profileID = \CSaleOrderUserProps::Update($profileID, $arFields)){
                $result->addErrorString(Module::get()->getMessage("save_error_save_fields"));
            } else {
        ...
        if (!empty($props) && !$result->getErrorsAsString()) {
            ...
            while ($arRes = $res->Fetch()) {
                if ($arRes['REQUIED'] === 'Y' && empty($props[$arRes['ID']])) {
                    $result->addErrorString(Module::get()->getMessage("save_error_required_field") . "\"" . $arRes["NAME"] . "\"");
                    continue;
                }
